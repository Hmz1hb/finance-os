  ...
  return (
    <>
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        <MetricCard label="Assets" value={formatMoney(assets)} tone="income" />
        <MetricCard label="Liabilities" value={formatMoney(liabilities)} tone="risk" />
        <MetricCard label="Net worth" value={formatMoney(assets - liabilities)} tone="plan" />
      </section>
      <section className="mt-4 grid gap-4 xl:grid-cols-2">
        <Card><CardHeader><CardTitle>Items</CardTitle></CardHeader><div className="space-y-2">{items.map((item) => <div key={item.id} className="flex justify-between rounded-md bg-surface-inset p-3"><span className="text-sm">{item.name}</span><span className="text-sm font-semibold">{formatMoney(item.madEquivalentCents)}</span></div>)}</div></Card>
        <Card><CardHeader><CardTitle>Snapshots</CardTitle></CardHeader><div className="space-y-2">{snapshots.map((snapshot) => <div key={snapshot.id} className="flex justify-between rounded-md bg-surface-inset p-3"><span className="text-sm">{snapshot.snapshotDate.toISOString().slice(0, 10)}</span><span className="text-sm font-semibold">{formatMoney(snapshot.netWorthCents)}</span></div>)}</div></Card>
      </section>
    </>
  );
}
